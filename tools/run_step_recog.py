...
def my_train_test_split(cfg, videos):
  ...
  if cfg.TRAIN.SPLIT_10P_TEST:
    ...
    if "M1" in cfg.SKILLS[0]["NAME"]:    
      ...
      videos, video_test = train_test_split(videos, test_size=0.10, random_state=2322) #M3 2343: only data until 07/09/2024; 2359: only data until june demo  1740: only with BBN pressure_videos.zip
      ...
      videos, video_test = train_test_split(videos, test_size=0.10, random_state=2250) #A8: 2303: only data until 10/11/2024; 2328 only data until 09/27/24; 2317: only data until 09/02/24; 2329: only with data until 07/31/2024; 1030: first test
      ...
      videos, video_test = train_test_split(videos, test_size=0.10, random_state=2345) #R16: 2315: only data until 10/03/2024; 2350 only data until 09/02/24; 1030: first test
